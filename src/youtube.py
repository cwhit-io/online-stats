...
class YouTubeLiveViewsFinder:
    ...
    def get_all_live_streams(self, max_results=2000):
    ...
    def find_stream_for_date(self, target_date, all_streams):
        ...
        if combined_stream:
            ...
            result['10:45am'] = 0
            result['notes'] = f"Combined stream ({combined_stream['duration']:.1f}h), recorded for 9am only"
            print(f"  > Combined stream ({combined_stream['duration']:.1f}h), recorded for 9am only")
            return result
        ...
        for ms in matching_streams:
            ...
            print(f"  DEBUG: Stream at {ms['local_start'].strftime('%I:%M %p')} (hour={start_hour:.2f})")
            ...
            # 10:45am service: starts between 10:00 AM-12:00 PM
                ...
                if service_1045am is None or ms['duration'] > service_1045am['duration']:
                    service_1045am = ms
            else:
        ...
        if service_9am:
            ...
            print(f"  > 9am service ({service_9am['duration']:.1f}h) - {service_9am['stream']['views']} views")
        
        if service_1045am:
            result['10:45am'] = service_1045am['stream']['views']
            notes_parts.append(f"10:45am: {service_1045am['duration']:.1f}h")
            print(f"  > 10:45am service ({service_1045am['duration']:.1f}h) - {service_1045am['stream']['views']} views")
        ...
        if notes_parts:
            ...
            result['notes'] = f"Unclear time: {best['local_start'].strftime('%I:%M %p')} ({best['duration']:.1f}h)"
            print(f"  > Unclear time: {best['local_start'].strftime('%I:%M %p')} ({best['duration']:.1f}h)")
    ...
    def process_attendance_csv(self, csv_file='attendance.csv', output_file=None):
        ...
        for index, row in df.iterrows():
            try:
                ...
                if (pd.notna(row.get('youtube 9am')) and str(row.get('youtube 9am')).strip() and 
                    pd.notna(row.get('youtube 1045am')) and str(row.get('youtube 1045am')).strip()):
        ...
        try:
            ...
            if self.discrepancy_log:
                ...
                print(f"WARNING: Logged {len(self.discrepancy_log)} discrepancies to: {log_file}")
